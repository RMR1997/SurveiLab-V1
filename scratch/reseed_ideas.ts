...
async function main() {
  console.log('Cleaning up existing StartupIdea records from Turso database...')
  
  const deleteResult = await prisma.startupIdea.deleteMany()
  console.log('Deleted StartupIdea rows:', deleteResult.count)

  console.log('Inserting StartupIdea records with correct client-side IDs...')
  const ideas = [
    {
      id: '1',
  ...
  console.log('Seeded 5 ideas with correct IDs!')
}
